...
resource "aws_s3_bucket_object" "client-opt" {
  bucket = "${aws_s3_bucket.frontend_ui.id}"
  key    = "client-opt.js"
  source = "${path.module}/../../client/target/scala-2.12/client-opt.js"
  ...
  etag = "${md5(file("${path.module}/../../client/target/scala-2.12/client-opt.js"))}"
}

resource "aws_s3_bucket_object" "client-jsdeps" {
  bucket = "${aws_s3_bucket.frontend_ui.id}"
  key = "client-jsdeps.js"
  source = "${path.module}/../../client/target/scala-2.12/client-jsdeps.js"
  content_type = "text/html"
  acl    = "public-read"
  etag = "${md5(file("${path.module}/../../client/target/scala-2.12/client-jsdeps.js"))}"
}
